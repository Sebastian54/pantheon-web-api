...
import { getAdvancements } from "@/lib/moderation";
...
export default async function AdvancementsPage() {
  ...
  const entries = await getAdvancements(visibleServers);
  ...
  return (
    <main className="mx-auto min-h-screen max-w-3xl space-y-6 p-8">
      <div>
        ...
        <h1 className="mt-2 text-2xl font-semibold tracking-tight">Advancements</h1>
      </div>
      <AdvancementsList entries={entries} />
    </main>
  );
}
